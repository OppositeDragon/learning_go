...
You will need to define and use a function called GenDisplaceFn() which takes three float64 arguments, acceleration a, initial velocity vo, and initial displacement so. GenDisplaceFn()
...
func main() {
	acc, iv, id := askValues()
	fn := genDisplaceFn(acc, iv, id)
	t := askTime()
	fmt.Println(fn(t))
}

func askValues() (float64, float64, float64) {
	var acc, iv, id float64
...
func genDisplaceFn(acc, iv, id float64) func(t float64) float64 {
	return func(t float64) float64 {
		return 0.5*acc*t*t + iv*t + id
	}
}

func askTime() float64 {
	var t float64
